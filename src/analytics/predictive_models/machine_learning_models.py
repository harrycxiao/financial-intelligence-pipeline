# src/analytics/predictive_models/machine_learning_models.py
...
MACHINE_LEARNING_OUTPUT_COLUMN = "machine_learning_expected_excess_return"


@dataclass
class MachineLearningExpectedReturnModel:
    """Container for trained ML expected-return models."""
...
def make_tree_pipeline(model: Any) -> Pipeline:
    """Create impute-only pipeline for tree models."""
...
def make_scaled_pipeline(model: Any) -> Pipeline:
    """Create impute-scale pipeline for models that need scaling."""
...
def train_random_forest_model(X: pd.DataFrame, y: pd.Series) -> Pipeline:
    ...
    pipeline = make_tree_pipeline(model)
    pipeline.fit(X, y)

    return pipeline


def predict_random_forest_returns(model: Pipeline, X: pd.DataFrame) -> np.ndarray:
    """Predict expected excess returns using Random Forest."""

    return np.asarray(model.predict(X), dtype=float).reshape(-1)


def train_extra_trees_model(X: pd.DataFrame, y: pd.Series) -> Pipeline:
    """Train Extra Trees regression."""
...
def predict_extra_trees_returns(model: Pipeline, X: pd.DataFrame) -> np.ndarray:
    """Predict expected excess returns using Extra Trees."""

    return np.asarray(model.predict(X), dtype=float).reshape(-1)


def train_hist_gradient_boosting_model(X: pd.DataFrame, y: pd.Series) -> Pipeline:
    """Train histogram gradient boosting regression."""
...
def predict_hist_gradient_boosting_returns(
    model: Pipeline,
    X: pd.DataFrame,
) -> np.ndarray:
    """Predict expected excess returns using histogram gradient boosting."""

    return np.asarray(model.predict(X), dtype=float).reshape(-1)


def train_mlp_model(X: pd.DataFrame, y: pd.Series) -> Pipeline:
    """Train small feed-forward neural network regression."""
    ...
    pipeline = make_scaled_pipeline(model)
    pipeline.fit(X, y)

    return pipeline


def predict_mlp_returns(model: Pipeline, X: pd.DataFrame) -> np.ndarray:
    """Predict expected excess returns using MLP."""

    return np.asarray(model.predict(X), dtype=float).reshape(-1)


def get_base_machine_learning_model_trainers() -> dict[str, Any]:
    """Return base ML model training functions."""

    return {
        "random_forest": train_random_forest_model,
        "extra_trees": train_extra_trees_model,
        "hist_gradient_boosting": train_hist_gradient_boosting_model,
        "mlp": train_mlp_model,
    }


def get_base_machine_learning_model_predictors() -> dict[str, Any]:
    """Return base ML model prediction functions."""

    return {
        "random_forest": predict_random_forest_returns,
        "extra_trees": predict_extra_trees_returns,
        "hist_gradient_boosting": predict_hist_gradient_boosting_returns,
        "mlp": predict_mlp_returns,
    }
...
def make_expanding_time_splits(
    ...
    dates = date_values.dropna().sort_values().unique()
...
def generate_out_of_fold_base_predictions(
    ...
    """Generate out-of-fold base ML predictions for meta-model training."""
...
def train_machine_learning_meta_model(
    oof_predictions: pd.DataFrame,
    target_column: str,
    base_model_names: list[str],
) -> Pipeline:
    """Train Ridge meta-model to combine base ML predictions."""

    clean = oof_predictions.dropna(subset=[target_column]).copy()

    if clean.empty:
        raise ValueError("No valid rows for ML meta-model training.")
    ...
    meta_model = make_scaled_pipeline(
...
def train_machine_learning_expected_return_model(
    training_df: pd.DataFrame,
    ...
    """Train stacked ML expected-excess-return model."""
    ...
    if not feature_columns:
        raise ValueError("No feature columns found for ML model training.")

    all_trainers = get_base_machine_learning_model_trainers()
    ...
    if not model_trainers:
        raise ValueError("No valid ML model names were provided.")
    ...
    meta_model = train_machine_learning_meta_model(
        oof_predictions=oof_predictions,
        target_column=target_column,
        base_model_names=base_model_names,
    )

    return MachineLearningExpectedReturnModel(
        feature_columns=feature_columns,
        target_column=target_column,
...
def predict_base_machine_learning_returns(
    model: MachineLearningExpectedReturnModel,
    current_features: pd.DataFrame,
) -> pd.DataFrame:
    """Predict expected excess returns from every base ML model."""

    X = current_features.loc[:, model.feature_columns].copy()

    predictors = get_base_machine_learning_model_predictors()
...
def predict_machine_learning_expected_returns(
    model: MachineLearningExpectedReturnModel,
    current_features: pd.DataFrame,
    output_column: str = MACHINE_LEARNING_OUTPUT_COLUMN,
) -> pd.DataFrame:
    """Predict final ML expected excess returns."""

    base_predictions = predict_base_machine_learning_returns(
        model=model,
...
def train_and_predict_machine_learning_expected_returns(
    training_df: pd.DataFrame,
    current_features: pd.DataFrame,
    feature_columns: Optional[list[str]] = None,
    target_column: str = "forward_excess_return",
    ...
    """Train ML model on historical rows, then predict current expected returns."""

    model = train_machine_learning_expected_return_model(
        training_df=training_df,
        feature_columns=feature_columns,
        target_column=target_column,
        date_column=date_column,
    ...
    return predict_machine_learning_expected_returns(
        model=model,
        current_features=current_features,
    )